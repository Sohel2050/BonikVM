...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  Future<void> _showRewardedAdForTime() async {
    try {
      ...
      final serverName = server?.name ?? 'VPN Server';
      ...
      final result = await showModalBottomSheet<bool>(
        ...
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
        builder: (_) => ExtendServerTimePopup(
          serverName: serverName,
          adCount: 3,
          durationMinutes: durationMinutes,
          onAdWatched: (index) async {
            final seconds = durationMinutes * 60;
            ref.read(freeConnectionTimerProvider.notifier).addTime(seconds);

            await ref
                .read(rewardVideoProvider.notifier)
                .markVideoWatched(index);
            ...
            // If the timer expired and the VPN was disconnected, reconnect
            // immediately after the first successful reward.
            final vpnState =
                ref.read(vpnStateProvider).value ?? VpnState.disconnected;
            final selectedServer = ref.read(currentServerProvider);
            if (vpnState == VpnState.disconnected && selectedServer != null) {
              try {
                await ref
                    .read(vpnServiceProvider)
                    .connectToServer(selectedServer);
              } catch (e) {
                debugPrint('⚠️ Reconnect after reward failed: $e');
              }
            }
          },
      ...
      if (result == null || result == false) {
        final timer = ref.read(freeConnectionTimerProvider);
        if (timer.remainingSeconds <= 0) {
          try {
            ref.read(vpnServiceProvider).disconnect();
          } catch (e) {
            debugPrint('❌ Disconnect after expired timer failed: $e');
          }
        }
      }
    } catch (e, st) {
      debugPrint('❌ ExtendServerTimePopup error: $e');
      debugPrint('$st');
    }
  }
